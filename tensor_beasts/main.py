...
from tensor_beasts.rl.iqn_trainer import IQNTrainer
...
class WorldThread(threading.Thread):
    ...
    def run(self):
        ...
        while not self.done:
            ...
            # world_stats["ups"] = ups
            self.update_screen_cb(step, {})
            step += 1


class WorldTrainerThread(threading.Thread):
    # TODO: Parametrization here is a total mess...
    # What should be the entry point? Is this pygame with trainer, or trainer with pygame?

    def __init__(self, world_size, update_screen_cb, clock, device):
        super().__init__(daemon=True)
    ...
    def run(self):
        ...
        trainer.train(update_screen_cb=self.update_screen_cb)
...
def main(args: argparse.Namespace):
    ...
    def update_screen_trainer(screen, step, world_stats):
        if not args.headless:
            display_manager.update_screen(screen)
        runtime_stats = {
            'current_screen': screens[current_screen_idx][0] if not args.headless else "(headless)",
            'fps': clock.get_fps(),
            'step': step
    ...
    # world_thread = WorldTrainerThread(args.size, update_screen_trainer, clock, args.device)
    world_thread.start()
